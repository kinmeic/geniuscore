...
-- Local Definations
---------------------
local MAJOR_VERSION = "LibUnit-1.0"
...
function lib:CanAttack(inCombat)
	if IsMounted() then return end	--在马上或载具上
...
function lib:CanBreak()
	...
	local spell = CastingInfo("target") or ChannelInfo("target")
	...
	if select(8, CastingInfo("target")) == false then
		return true
	elseif select(7, ChannelInfo("target")) == false then
		return true
	end
end
...
function lib:Casting()
	return CastingInfo(self.unit) or ChannelInfo(self.unit) or SpellIsTargeting()
end

-- 施法剩余时间
function lib:GetCastingTime()
	local name, _, _, _, startTime, endTime = CastingInfo(self.unit)
