...
local Frostbolt = LibSpell:NewByName("寒冰箭")
local FireBlast = LibSpell:NewByName("火焰冲击")
...
-------------------
-- 刺杀
Core.Scheme[0] = function()
	if (not Switch2:GetChecked()) and Target:CanBreak() and KK:CanCast() then	--脚踢
		return "S="
	elseif FireBlast:CanCast() then	--火焰冲击
		return "D3"
	elseif Frostbolt:CanCast() then	--寒冰箭
		return "D2"
	end
end
